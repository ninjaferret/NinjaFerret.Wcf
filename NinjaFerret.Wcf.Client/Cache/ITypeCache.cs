...
namespace NinjaFerret.Wcf.Client.Cache
{
    ...
    {
        bool Contains<TServiceType>() where TServiceType : class;
        void AddType<TServiceType>(Type client) where TServiceType : class;
        Type GetType<TServiceType>() where TServiceType : class;
    }
}